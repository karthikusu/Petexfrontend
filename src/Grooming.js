...
const Grooming = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:1114/api/grooming/all')
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:1114/api/grooming/${editItem.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = (itemId, itemName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:1114/api/grooming/${itemId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
  ...
  return (
    <div className="containergrooming">
      <h2 className='title'>Grooming Services List</h2>
      <Link to="/groomingform">
              ...
              <td>
                {editItem === item ? (
                  ...
                  >
                    ...
                    <option value="Anti-Thick">Anti_thick tratment</option>
