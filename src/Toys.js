...
function Toys() {
  ...
  useEffect(() => {
    fetch('http://localhost:1111/api/toys')
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:1111/api/toys/${editProduct.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = (productId, productName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:1111/api/toys/${productId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
  ...
  return (
    <div className="body">
      ...
      <div className="containertoys">
        ...
        <div className="table-responsive">
          <Link to="/toysvendorform"><button className="button">+ Add New Items</button></Link>
          <h2 className='title'>Toys Items List</h2>
