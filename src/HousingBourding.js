...
import React, { useState, useEffect } from 'react';
  ...
  useEffect(() => {
    fetch('http://localhost:1115/house/board/all')
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:1115/house/board/${editItem.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = (itemId, itemName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:1115/house/board/${itemId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
