...
function PetInfoForm1() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:9009/api/v1/save/${userId}/${doctId}`, pojoData);
      ...
      if (res === "Appointment successfully booked") {
        setUnavailableSlots([]);
        setBookedSlots([]);
         ...
         {allTimeSlots.map((time, index) => {
          ...
          <div className="form-group">
            ...
            <select id="typeOfService" name="typeOfService">
              ...
              <option value="Physical Consulting">Home Visit</option>
            </select>
          </div>
