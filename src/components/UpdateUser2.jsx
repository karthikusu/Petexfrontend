...
function UpdateUser() {
 ...
 const handleSubmit = async (e) => {
  ...
  if (isValid) {
    try {
      const response = await axios.put(`http://localhost:9000/api/users/update/${userId}`, {
        petname,
        petType,
        breed,
        species,
  ...
  return (
    <div className='backgroundContainer4' style={{ backgroundImage: `url(${BG2})`, height: "1200px" }}>
      <div className='formContainer4'>
        <form onSubmit={handleSubmit} style={{ backgroundColor: 'transparent', borderRadius: '8px', padding: '20px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.6)', width: "700px",height: "130vh", marginLeft: "10%", marginTop: "0%", color: "white"}}>
          <h2>Update User</h2>

          <div style={{ display: 'flex', marginBottom: '10px' }}>
            <div style={{ marginRight: '10px', flex: 1 }}>
              <label htmlFor="disabledSelect" className="form-label" style={{marginTop:"-3%"}}>PetType :</label>
              <select id="disabledSelect" className="form-select" name="petType" value={petType} onChange={handleChange} style={{ width: "300px", height: "45px", borderRadius: '4px', border: '1px solid #ccc' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            <button type="submit" className="btn btn-primary" style={{ width: '150px', padding: '10px', borderRadius: '6px', border: 'none', backgroundColor: '#1e88e5', cursor: 'pointer', marginLeft:"280px"}}>Update</button>
          </div>
        </form>
      </div>
    </div>
