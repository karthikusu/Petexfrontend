...
function Clothing() {
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:1113/api/cloth/${editProduct.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = (productId, productName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:1113/api/cloth/${productId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
  ...
  return (
    <div className="body">
      ...
      <div className="containercl">
        ...
        <div className="table-responsive">
          <Link to="/clothingform"><button className="button">+ Add New Items</button></Link>
          <h2 className='title'>Clothing Items List</h2>
              ...
              {Array.isArray(products) && products.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="text-center">
                    {editProduct === product ? (
                      <>
                        <button className="btn btn-primary1" onClick={handleUpdate}>
                          Update
                        </button>
                        <button className="btn btn-primary2" onClick={() => setEditProduct(null)}>
                          Cancel
                        </button>
                      </>
                    ) : (
