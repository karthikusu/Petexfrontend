...
import { Link } from 'react-router-dom';
  ...
  useEffect(() => {
    fetch('http://localhost:1116/api/daycare/all')
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:1116/api/daycare/${editItem.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = (itemId, itemName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:1116/api/daycare/${itemId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
